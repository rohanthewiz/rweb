...
func (c *Cookie) AppendBytes(dst []byte) []byte {
	...
	if c.maxAge != 0 {
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieMaxAge...)
		dst = append(dst, '=')
		if c.maxAge < 0 {
		...
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieExpires...)
		dst = append(dst, '=')
		dst = append(dst, c.bufV...)
	}
	if len(c.domain) > 0 {
		dst = appendCookiePart(dst, consts.StrCookieDomain, c.domain)
	}
	if len(c.path) > 0 {
		dst = appendCookiePart(dst, consts.StrCookiePath, c.path)
	}
	if c.httpOnly {
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieHTTPOnly...)
	}
	if c.secure {
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieSecure...)
	}
	switch c.sameSite {
	case CookieSameSiteDefaultMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieSameSite...)
	case CookieSameSiteLaxMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieSameSite...)
		dst = append(dst, '=')
		dst = append(dst, consts.StrCookieSameSiteLax...)
	case CookieSameSiteStrictMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieSameSite...)
		dst = append(dst, '=')
		dst = append(dst, consts.StrCookieSameSiteStrict...)
	case CookieSameSiteNoneMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookieSameSite...)
		dst = append(dst, '=')
		dst = append(dst, consts.StrCookieSameSiteNone...)
	}
	if c.partitioned {
		dst = append(dst, ';', ' ')
		dst = append(dst, consts.StrCookiePartitioned...)
	}
	return dst
}
...
func (c *Cookie) ParseBytes(src []byte) error {
	...
	for s.next(&c.bufK, &c.bufV) {
		if len(c.bufK) != 0 {
			...
			case 'm':
				if caseInsensitiveCompare(consts.StrCookieMaxAge, c.bufK) {
			...
			case 'e': // "expires"
				if caseInsensitiveCompare(consts.StrCookieExpires, c.bufK) {
					v := b2s(c.bufV)
			...
			case 'd': // "domain"
				if caseInsensitiveCompare(consts.StrCookieDomain, c.bufK) {
					c.domain = append(c.domain, c.bufV...)
				}

			case 'p': // "path"
				if caseInsensitiveCompare(consts.StrCookiePath, c.bufK) {
					c.path = append(c.path, c.bufV...)
				}

			case 's': // "samesite"
				if caseInsensitiveCompare(consts.StrCookieSameSite, c.bufK) {
					if len(c.bufV) > 0 {
						// Case insensitive switch on first char
						switch c.bufV[0] | 0x20 {
						case 'l': // "lax"
							if caseInsensitiveCompare(consts.StrCookieSameSiteLax, c.bufV) {
								c.sameSite = CookieSameSiteLaxMode
							}
						case 's': // "strict"
							if caseInsensitiveCompare(consts.StrCookieSameSiteStrict, c.bufV) {
								c.sameSite = CookieSameSiteStrictMode
							}
						case 'n': // "none"
							if caseInsensitiveCompare(consts.StrCookieSameSiteNone, c.bufV) {
			...
			switch c.bufV[0] | 0x20 {
			case 'h': // "httponly"
				if caseInsensitiveCompare(consts.StrCookieHTTPOnly, c.bufV) {
					c.httpOnly = true
				}

			case 's': // "secure"
				if caseInsensitiveCompare(consts.StrCookieSecure, c.bufV) {
					c.secure = true
				} else if caseInsensitiveCompare(consts.StrCookieSameSite, c.bufV) {
					c.sameSite = CookieSameSiteDefaultMode
				}
			case 'p': // "partitioned"
				if caseInsensitiveCompare(consts.StrCookiePartitioned, c.bufV) {
					c.partitioned = true
				}
			}
